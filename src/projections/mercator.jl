...
function Mercator(;long0::AbstractFloat=0.0f0, k::AbstractFloat=1.0f0, radius::AbstractFloat=MEAN_RADIUS_WGS_84) 
    Mercator(promote(radius * k, long0)...)
end

inv(proj::Mercator) = InverseMercator(proj.radius, proj.long0)
...
function project(proj::Mercator{T1}, coordinate::Tuple{T2, T2}; atol=1e-6) where {T1,T2 <: AbstractFloat}
    ...
    y = at_pole ? T1(Inf) : proj.radius * log(tan(angle))
    (x, y)
end

"""
    InverseMercator(radius, long0)
...
inv(proj::InverseMercator) = InverseMercator(proj.radius, proj.long0)
